...
from flask_app.demo_scripts.demo_thermal_imaging.read_is2_new.otherfunctions import *
import numpy as np
...
def read_is2_new(filename):
        ...
        if cal_data[i] == 74 and cal_data[i + 1] == 25 and cal_data[i + 2] == 13:
            ...
            if temp_range[0] >= -180:
                ...
                for j in range(data_range_int[0], data_range_int[1]):
                    # Fluke uses a quadratic function with temperature as input, and IR-data as output.
                    # We want data as input and temperature as output, so the abc-equation is used.
                    ir['conversion'][j] = (-equation_variables['b'] + np.sqrt(
                        equation_variables['b'] ** 2 - 4 * equation_variables['a'] * (equation_variables['c'] - j))) / (
    ...
    for i in d[index:index + (ir['size'][0] * ir['size'][1])]:
        raw_temp.append(
            (ir['conversion'][i] - (2 - ir['transmission'] - ir['emissivity']) * ir['backgroundtemperature']) / (
                    ir['transmission'] * ir['emissivity']))
